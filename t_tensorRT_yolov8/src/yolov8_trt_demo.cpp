...
void YOLOv8TRTDetector::detect(cv::Mat &frame, std::vector<DetectResult> &results) {
	...
	float t0 = (cv::getTickCount() - start0) / static_cast<float>(cv::getTickFrequency());
	std::cout << "初始化时间: " << t0*1000.0 << " 毫秒" << std::endl;
	int64 start1 = cv::getTickCount();
	...
	float t1 = (cv::getTickCount() - start1) / static_cast<float>(cv::getTickFrequency());
	std::cout << "传输 + 推理时间: " << t1*1000.0 << " 毫秒" << std::endl;
	...
	// 计算FPS render it
	float t2 = (cv::getTickCount() - start2) / static_cast<float>(cv::getTickFrequency());
	// cv::putText(frame, cv::format("FPS: %.2f", 1.0 / t), cv::Point(20, 40), cv::FONT_HERSHEY_PLAIN, 2.0, cv::Scalar(255, 0, 0), 2, 8);

	std::cout << "后处理时间: " << t2 *1000.0 << " 毫秒" << std::endl;
}
